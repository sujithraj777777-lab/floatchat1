...
def calculate_salinity_change(
    ...
    """Calculate salinity change between two cycles."""
    ...
    sals_a = np.array([o["salinity_psu"] for o in obs_a])
    sals_b = np.array([o["salinity_psu"] for o in obs_b])
    ...
    for i, ob in enumerate(obs_b):
        ...
        if abs(pressures_a[closest] - ob["pressure_dbar"]) < 50:
            delta = ob["salinity_psu"] - obs_a[closest]["salinity_psu"]
            matched.append({
                "pressure_dbar": ob["pressure_dbar"],
                "depth_m": ob["depth_m"],
                "sal_a": obs_a[closest]["salinity_psu"],
                "sal_b": ob["salinity_psu"],
                "delta_sal": float(delta),
            })

    deltas = [m["delta_sal"] for m in matched]
    ...
    return {
        ...
        "summary": {
            ...
            "max_increase": float(np.max(deltas)) if deltas else None,
            "max_decrease": float(np.min(deltas)) if deltas else None,
